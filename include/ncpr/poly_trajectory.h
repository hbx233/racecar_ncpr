...
namespace ncpr{
  ...
  private:
    ...
    PolyTrajectory(Eigen::Matrix<T, OutputDim, BasisOrder+1> coeff, double total_time)
        : coeff_(coeff), total_time_(total_time) {};
  public:
    //Define Useful types
    ...
    using Ptr = unique_ptr<PolyTrajectory<T,OutputDim,BasisOrder>>;
    /*!
    ...
    OutputType getDesired(double time, int order);
    /*!
     ...
     */  
    nav_msgs::Path generatePath(double time_interval);
    /*!
     ...
     */
    void fitPolyTrajectory(OutputType start, OutputType goal, OutputType start_vel, OutputType goal_vel, double total_time);
  private:
    /*!
     ...
     */
    BasisType getBasisVector(double time, int order);
    OutputType  start_;
    OutputType goal_;
    double total_time_;
    CoeffType coeff_;
...
template <typename T, int OutputDim, int BasisOrder> 
PolyTrajectory::OutputType PolyTrajectory<T, OutputDim, BasisOrder>::getDesired(double time, int order)
{
...
template <typename T, int OutputDim, int BasisOrder>
nav_msgs::Path PolyTrajectory<T, OutputDim, BasisOrder>::generatePath(double time_interval){
  double t = 0;
  nav_msgs::Path path;
  //TODO: Need Header??
  while(t<total_time_){
...
template <typename T, int OutputDim, int BasisOrder>
void PolyTrajectory<T, OutputDim, BasisOrder>::fitPolyTrajectory(OutputType start, OutputType goal, OutputType start_vel, OutputType goal_vel, double total_time){
...
template <typename T, int OutputDim, int BasisOrder> 
PolyTrajectory::BasisType PolyTrajectory<T, OutputDim,BasisOrder>::getBasisVector(double time, int order){
